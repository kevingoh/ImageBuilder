...
class App_service_email_Admin
{
    ...
    private function fetch_total_logs_and_counts($wpdb, $table_name)
    {
        $total_logs = $wpdb->get_results("SELECT status, COUNT(*) as count FROM $table_name GROUP BY status");
        ...
        foreach ($total_logs as $log) {
            $total_counts['total_count'] += $log->count;
            if ($log->status === 'Success') {
                $total_counts['success_count'] = $log->count;
            } elseif ($log->status === 'Failure') {
                $total_counts['failure_count'] = $log->count;
            }
        }
